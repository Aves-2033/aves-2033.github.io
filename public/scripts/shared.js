...
function cleanupModalHandlers() {
  ...
  if (eventHandlers.modalImageClick) {
    ...
    if (modalImageContainer) {
      modalImageContainer.removeEventListener(
        "click",
        eventHandlers.modalImageClick,
      );
      modalImageContainer.removeEventListener(
        "mousemove",
        eventHandlers.modalImageMouseMove,
      );
      modalImageContainer.removeEventListener(
        "mouseleave",
      ...
      if (eventHandlers.modalGalleryTouchStart) {
        modalImageContainer.removeEventListener(
          "touchstart",
          eventHandlers.modalGalleryTouchStart,
        );
        modalImageContainer.removeEventListener(
          "touchmove",
          eventHandlers.modalGalleryTouchMove,
        );
        modalImageContainer.removeEventListener(
          "touchend",
...
function cleanupLightboxHandlers() {
  ...
  if (eventHandlers.lightboxBackground) {
    ...
    if (lightboxModal) {
      lightboxModal.removeEventListener(
        "click",
        eventHandlers.lightboxBackground,
      );
  ...
  if (lightboxModal) {
    if (eventHandlers.lightboxTouchStart) {
      lightboxModal.removeEventListener(
        "touchstart",
        eventHandlers.lightboxTouchStart,
      );
    }
    if (eventHandlers.lightboxTouchMove) {
      lightboxModal.removeEventListener(
        "touchmove",
        eventHandlers.lightboxTouchMove,
      );
    }
    if (eventHandlers.lightboxTouchEnd) {
      lightboxModal.removeEventListener(
        "touchend",
        eventHandlers.lightboxTouchEnd,
      );
    }
  }
...
function cleanupContactModalHandlers() {
  if (eventHandlers.contactModalClose) {
    ...
    if (contactCloseBtn)
      contactCloseBtn.removeEventListener(
        "click",
        eventHandlers.contactModalClose,
      );
    if (contactModal)
      contactModal.removeEventListener(
        "click",
        eventHandlers.contactModalBackground,
      );
...
function openContactModal() {
  ...
  if (contactCloseBtn)
    contactCloseBtn.addEventListener("click", eventHandlers.contactModalClose);
...
function openLightbox(index, imageList, imageAlt) {
  ...
  if (lightboxClose)
    lightboxClose.addEventListener("click", eventHandlers.lightboxClose);
  lightboxModal.addEventListener("click", eventHandlers.lightboxBackground);
  lightboxModal.addEventListener("wheel", eventHandlers.lightboxWheel, {
    passive: false,
  });
  ...
  lightboxModal.addEventListener("touchmove", eventHandlers.lightboxTouchMove, {
    passive: true,
  });
  lightboxModal.addEventListener("touchend", eventHandlers.lightboxTouchEnd, {
    passive: true,
  });
  ...
  if (hasMultipleImages) {
    ...
    thumbnails.forEach((thumb, idx) => {
      thumb.addEventListener("click", () =>
        switchImage(idx, images, modalImage, thumbnails),
      );
    });
  }
